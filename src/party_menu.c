...
static void DisplayPartyPokemonDataForMultiBattle(u8 slot)
{
    ...
    else
    {
        ...
        DisplayPartyPokemonBarDetailToFit(menuBox->windowId, gStringVar1, 0, menuBox->infoRects->dimensions, menuBox->infoRects->dimensions[2]);
...
static void DisplayPartyPokemonNickname(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        DisplayPartyPokemonBarDetailToFit(menuBox->windowId, nickname, 0, menuBox->infoRects->dimensions, menuBox->infoRects->dimensions[2]);
    }
}
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        const u8 *text;
        u8 fontColorsId = 3;
        u8 usableWidth = GetWindowAttribute(sPartyMenuInternal->windowId[0], WINDOW_WIDTH) * 8 - cursorDimension;
        u8 textX;
        ...
        textX = cursorDimension + GetStringCenterAlignXOffset(FONT_NORMAL, text, usableWidth);
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], FONT_NORMAL, textX, (i * 16) + 1, letterSpacing, 0, sFontColorTable[fontColorsId], 0, text);
    }
